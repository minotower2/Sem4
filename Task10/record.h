...
class record {
  ...
  public:
    ...
    io_status read (FILE *fp = stdin) {
      ...
      if (group >= 1000) group = group % 1000;
      ...
      if (keys[0] == '2') {
        ...
        if (keys[strlen(keys)-1] == '2') {
          ...
          for (int b = 0; buffer[j]; b++, j++) {
            ...
            if (keys[j] == '3') {
              if (x[dif+b] > buffercopy[j] || x[dif+b] < buffer[j]) {
                return false;
              }
            }
            if (keys[j] == '4') {
  ...
  public:
    void writeToServer (int fd, const ordering order[] = nullptr) {
      int nbytes, len, i;
      char buf[LEN] = {0};
      // Длина сообщения
      const int max_items = 3;
      ...
      for (int i = 0; i < max_items; i++) {
        switch (p[i]) {
          case ordering::name: {
            int leng = strlen(buf);
            snprintf(buf + leng, LEN - leng, " %s", name.get());
            break; }
          case ordering::phone: {
            int leng = strlen(buf);
            snprintf(buf + leng, LEN - leng, " %d", phone);
            break; }
          case ordering::group: {
            int leng = strlen(buf);
            snprintf(buf + leng, LEN - leng, " %d", group);
            break; }
          case ordering::none:
            continue;
        }
