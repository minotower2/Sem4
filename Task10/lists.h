  ...
  public:
    ...
    bool delete_parse(const char *string, config con) {
      ...
      if (res == 1) {
        ...
        list1 tempqueue;
        list4 * nodes = tab.find_value(&buff, con);
...
phone:
        list3 * nodess = tabp.find_value(&buff, con);
...
checking:
        ...
        list4 * nodes = tabg.find_value_name(&buff, con);
        if (nodes == nullptr) return true; 
        if (nodes->get_body() && ((nodes->get_body())->get_del() == false) && buff.apply(*(nodes->get_body()))) {
          (nodes->get_body())->set_del(true);
        }
        ...
        list3 * nodes = tabg.find_value_phone(&buff, con);
        if (nodes == nullptr) return true;
        list1_node * cur;
        if (nodes->get_body() && ((nodes->get_body())->get_del() == false) && buff.apply(*(nodes->get_body()))) {
          (nodes->get_body())->set_del(true);
      ...
      /*
        ...
        list1 tempqueue;
        tabg.add_all_to(tempqueue, buff.get_group());
        if (tempqueue.get_head() == nullptr) return 0;
        for (list1_node *cur = tempqueue.get_head(); cur; cur = cur->get_next()) {
          if (cur->get_body() && ((cur->get_body())->get_del() == false) && buff.apply(*(cur->get_body()))) {
            (cur->get_body())->set_del(true);
          }
        }
...
phone:
        list3 * nodess = tabp.find_value(&com, con);
        if (nodess == nullptr) goto checking;
        list1_node * curr;
...
checking:
      ...
      /*
        ...
        list1 tempqueue;
        tabg.add_all_to(tempqueue, com.get_group());
        if (tempqueue.get_head() == nullptr) return 0;
        if ((tempqueue.get_head())->get_next() == nullptr) {
          if ((tempqueue.get_head()) && (((tempqueue.get_head())->get_body())->get_del() == false) && com.apply(*((tempqueue.get_head())->get_body()))) {
            queue.add_node((tempqueue.get_head())->get_body());
            count++;
          }
        }
          ...
          tempqueue.merge_sort(orders);
          //printf("tempqueue: \n");
          //tempqueue.print_list(orders);
          //printf("\n");
          list1_node *prev = tempqueue.get_head();
          for (list1_node *curr = (tempqueue.get_head())->get_next(); curr; curr = curr->get_next()) {
            if ((curr->get_body())->is_eq(*(prev->get_body())) == false) {
              if ((curr->get_body()->get_del() == false) && com.apply(*(curr->get_body()))) {
                queue.add_node(curr->get_body());
                count++;
              }
            }
            prev = curr;
          }
          if ((prev->get_body()->get_del() == false) && com.apply(*(prev->get_body()))) {
            queue.add_node(prev->get_body());
            count++;
          }
        }
